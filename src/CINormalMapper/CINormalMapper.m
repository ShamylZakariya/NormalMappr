...
- (void)setSize:(CGSize)newSize
{
    ...
    if (((int)newSize.width != (int)size.width) || ((int)newSize.height != (int)size.height)) {
        size = newSize;
        dirty = YES;
    }
}
...
- (void)prepareBuffers
{
    ...
    if (!normalmap || ABS(self.size.width - [normalmap size].width) > 0.0f || ABS(self.size.height - [normalmap size].height) > 0.0f) {
        //
        // create normalmap
        //
    ...
    //
    if (!inputImage || ABS(self.size.width - [inputImage extent].size.width) > 0.0f || ABS(self.size.height - [inputImage extent].size.height) > 0.0f) {
